...
-C++が実行時にオブジェクトの本当の型を調べるための言語機能のこと
-動的型に関する情報を実行時に扱う
...
### ポインタ型と参照型の使い分け
変換先をA*にするか、A&にするかで失敗時の表現が変わる

- Aではない可能性を、普通の結果として扱うならポインタ型
- 必ずAであるはず、で違っていたら異常として扱うなら参照型
...
## catch書き方メモ
書き方	                       捕まえる例外	　　catch内で例外を使えるか
catch (const std::exception& e)	同じ	　　　　e.what()などが使える
catch (const std::exception&)	同じ	　　　　使えない